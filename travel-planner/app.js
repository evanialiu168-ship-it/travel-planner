...
async function generatePlan(){
  ...
  resultDiv.innerHTML = `<div class="p-5 text-center text-gray-500 font-semibold">正在生成${city}的${days}天旅行计划，请稍候…</div>`;
  ...
  try{
    ...
    if(!res.ok){
      ...
      throw new Error(errData.error || "服务器返回错误");
    }
    ...
    let html = `<div class="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-5 rounded-2xl mb-5">
                  ...
                  <p class="mt-2 text-xl font-bold">💰预计预算: ¥${plan.days.reduce((a,b)=>a+b.budget,0).toLocaleString()}</p>
                </div>`;
    ...
    resultDiv.innerHTML = `<div class="p-5 text-center text-red-500 font-semibold">生成行程失败：${err.message}</div>`;
  }
}