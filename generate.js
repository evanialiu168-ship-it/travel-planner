...
const openai = new OpenAI({
  apiKey: apiKey || "dummy-key-to-prevent-crash", // 如果全是空的，给个假 Key 防止初始化直接 Crash
...
export default async function handler(req, res) {
  ...
  // 如果根本没有配置 Key，直接温柔地返回给前端，而不是让服务器 Crash
  if (!apiKey) {
    return res.status(500).json({ 
      error: "AI配置错误", 
      details: "服务器上没有找到任何 API Key，请检查 Vercel 的 Environment Variables 设置！" 
    });
  }

  // 后续的 const { city, days, spots } = req.body; ... 保持完全不变